...
seu_ISTUnsup <- readRDS(here("output","processed_data","SCT","seu_SCT_ISTclus.RDS"))

# Load Seurat-Louvain Seu object
seu_SL <- readRDS(here("output","processed_data","SCT","seu_SCT_SLclus.RDS"))
...
n_ISTUnsup <- length(unique(Idents(seu_ISTUnsup)))
cols_ISTUnsup <- gg_color_hue(n_ISTUnsup)

n_SL <- length(unique(Idents(seu_SL)))
cols_SL <- gg_color_hue(n_SL)

# DimPlots of each clustering method
p1 <- DimPlot(seu_ISTSup, reduction = "umap", label = TRUE, cols = cols_ISTSup, 
              raster = FALSE) +
  ggtitle("Supervised InSituType")

p2 <- DimPlot(seu_IST, reduction = "umap", label = TRUE, cols = cols_ISTUnsup,
              raster = FALSE) +
  ggtitle("Unsupervised InSituType + ScType")

p3 <- DimPlot(seu_SL, reduction = "umap", label = TRUE, cols = cols_SL,
              raster = FALSE) +
...
1.  [**Supervised InSituType**](4.0_insitutype_cell_typing.html)
...
```{r Sup, echo = FALSE, fig.width = 7, fig.height = 7}
p1
```

2.  [**Unsupervised InSituType**](4.1_insitutype_unsup_clustering.html)
...
```{r Unsup, echo = FALSE, fig.width = 7, fig.height = 7}
p2
```

3.  [**Seurat's unsupervised clustering, using the Louvain algorithm**](4.2_seurat_unsup_clustering.html)
...
In terms of time and memory usage, methods...
